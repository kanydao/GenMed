"""Standard MSN data loader for category-conditioned training and evaluation."""
...
from dataset._helpers import augment_sdf, data_collate, get_prompt
...
class MsnDataset(Dataset):
    """MSN dataset returning (sdf_tensor, text_embedding, category, prompt_dict)."""
...
class _TypeBatchSampler:
    """Yields per-class batches; resamples short classes to fill the batch size."""

    def __init__(self, dataset: MsnDataset, batch_size: int, training: bool):
...
def get_loader(cfg, mode: str = "train", augment: bool = False) -> DataLoader:
    assert mode in ("train", "test", "uncond")
    dataset = MsnDataset(cfg, mode=mode, augment=augment)
